...
6. On `COMPLETED`: pull the consolidated summary (`--order DESC --max-items 10`), then `aws___call_aws(cli_command="aws devops-agent list-recommendations --agent-space-id SPACE_ID --task-id TASK_ID --region us-east-1")` → `aws___call_aws(cli_command="aws devops-agent get-recommendation --agent-space-id SPACE_ID --recommendation-id REC_ID --region us-east-1")` for each. Show the user the proposed fix; **do not** auto-apply IaC or commands.
