...
3. `aws___call_aws(cli_command="aws devops-agent create-chat --agent-space-id SPACE_ID --region us-east-1")` → `aws___run_script` with `send_message(executionId, content="<IaC context>\n\nAnalyze cost optimization opportunities. $ARGUMENTS")`.
